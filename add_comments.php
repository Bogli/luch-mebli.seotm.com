...
if (!isset($_REQUEST['id_module'])) $id_module = NULL;
else $id_module = FrontForm::GetRequestNumData($_REQUEST['id_module']);

$Obj = new FrontComments(NULL, $id_module);
...
$Obj->script = '/add_comments.php?id_module='.$Obj->module.'&id_item='.$Obj->id_item;
...
switch($Obj->task){
    ...
    case 'add_comment':
        if( $Obj->CheckFields()!=NULL ){
            $Obj->ShowCommentsFormHTML();
        }
        else {
            if( $Obj->SaveComments()) {
                ?><div style="width:100%;"><?=$Obj->Form->ShowTextMessages($Obj->multi['TXT_FRONT_COMMENT_SAVE_OK']);?></div>
                <script>ReloadComments('<?=$Obj->script;?>', 'commentsBlock');</script><?
                $Obj->SendResponseEmail();
            }
            else{
                $Obj->Err = $Obj->multi['MSG_FRONT_ERR_COMMENT_NOT_SAVE'];
                $Obj->ShowCommentsFormHTML();
            }
        }
        break;
    ...
    case 'show_comments':
        //$Obj->ShowCommentsList();
        $Obj->ShowCommentsTree(10);
        break;
