...
class News {
    ...
    */
    ...
    {
        $q = "SELECT `".TblModNewsLinks."`.`id` FROM `".TblModNewsLinks."` WHERE `link`='".$link."'";
        $res = $this->db->db_Query( $q );
        if( !$res OR !$this->db->result) return false;
    ...
    $link = $Crypt->GetTranslitStr($cat_link);
    if($this->IsLink($link)) $link .= '-'.$cod;
    ...
    function GetLink($cod){
      $q = "select `link` from ".TblModNewsLinks." where `cod`='".$cod."'";
      $res = $this->db->db_Query( $q );
      $rows = $this->db->db_GetNumRows();
   ...
   //echo $body;
    ...
    function QuickSearch($search_keywords)
    {
     ...
     // Using in FrontComments.class.php
