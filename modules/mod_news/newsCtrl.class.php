...
function save()
{
    ...
    if( $rows>0 )   //--- update
    {
     ...
     if( $res===false OR !empty($res) ) return false;
     if( isset($this->settings['rss_import']) AND $this->settings['rss_import']==1){
...
function del( $id_del )
{
  ...
  return $del;
    ...
    function down( $move )
    {
     ...
     if( $move_down!=0 AND $move_up!=0 )
     {
     $q="update ".TblModNews." set display='$move_down' where id='$id_up'";
     //echo '<br>'.$q;
     $res = $this->Right->Query( $q, $this->user_id, $this->module );

     $q="update ".TblModNews." set display='$move_up' where id='$id_down'";
     //echo '<br>'.$q;
     $res = $this->Right->Query( $q, $this->user_id, $this->module );
     }
    }
            ...
            <TABLE border=0 cellpadding=2 cellspacing=0 class="EditTable">
             <TR>
              ...
              <TD align=center valign="middle" width="255">
               <a href="<?=$this->GetImgPath($row['path'], $this->id);?>" target=_blank><?=$this->ShowImage($row['path'], $this->id, 'size_auto=250', 100, NULL, "border=0");?></a>
              </TD>
              <TD valign="top">
