...
// ================================================================================================
// System : SEOCMS
// Module : gallery_settings.class.php
// Version : 1.0.0
// Date : 01.07.2010
// Licensed To:
// Yaroslav Gyryn ihoru@mail.ru
// Yaroslav Gyryn las_zt@mail.ru
//
// Purpose : Class definition for all actions with settings of Gallery
//
// ================================================================================================
...
//    Returns           : None                                               
//    Description       : Class definition for all actions with managment of Gallery
// ================================================================================================
...
// ================================================================================================
 class Gallery_settings extends Gallery {
    var $set_keywrd;
    var $set_descr;
    var $rss;

    // ================================================================================================
    //    Function          : Gallery_settings (Constructor)
    //    Version           : 1.0.0
    //    Date              : 01.07.2010
    ...
    function __construct($user_id=NULL, $module=NULL, $display=NULL, $sort=NULL, $start=NULL, $width = NULL) {
            ...
            if (empty($this->Form)) $this->Form = new Form('form_mod_gallery');
            if (empty($this->Spr)) $this->Spr = new  SysSpr($this->user_id, $this->module);
    } // End of Gallery_settings Constructor
    ...
    // Description : show setting of Galleryue
    // ================================================================================================
    ...
    $script = $_SERVER['PHP_SELF'].'?module='.$this->module;
     ...
     $tmp_bd= new DB();
      $q1="select * from `".TblModGallerySetSprMeta."` where 1";
...
<?}?>
    ...
    else
    {
      ...
      $q="insert into `".TblModGallerySet."` values('$this->img', '$this->set_keywrd', '$this->set_descr', '$this->rewrite', '$this->dt', '$this->img_path', '$this->rss')";
      $res = $this->Right->Query( $q, $this->user_id, $this->module );
      //echo '<br>q='.$q.' res='.$res.' $this->Right->result='.$this->Right->result; 
      if( !$this->Right->result) return false;
    }
////////////////////////////////////////////////////////////    
    $q="select * from `".TblModGallerySetSprMeta."` where 1";
    $res = $this->Right->Query( $q, $this->user_id, $this->module );
    if( !$this->Right->result ) return false;
    $rows = $this->Right->db_GetNumRows();
    $meta= array();
     ...
     $ln_sys = new SysLang();
            ...
            while( $el = each( $ln_arr ) )
            {
              ...
              if (isset($meta[$lang_id]))
              {
                 ...
                 $q="update `".TblModGallerySetSprMeta."` set
...
//                  echo '<br>q='.$q.' res='.$res.' $this->Right->result='.$this->Right->result; 
        ...
        {
          $q="insert into `".TblModGallerySetSprMeta."`  set
                  `title`='".$this->title[$lang_id]."',
                  `description`='".$this->description[$lang_id]."',
                  `keywords`='".$this->keywords[$lang_id]."',
                  `lang_id`='$lang_id'";
