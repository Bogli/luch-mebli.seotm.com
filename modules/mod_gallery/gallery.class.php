...
// end of Gallery (Constructor)
    ...
    function GetGallerysRows($limit='limit')
    {
        ...
        SELECT
	     ...
	     `mod_gallery_img`.`path` AS `img`,
	     `mod_gallery_img_spr`.`name` AS `alt`,
	     `mod_gallery_img_spr`.`text` AS `title`
        FROM `".TblModGallery."`
	    LEFT JOIN `mod_gallery_img` ON (`mod_gallery_img`.`id_position`=`".TblModGallery."`.`id` AND `mod_gallery_img`.`visible`='1' AND `mod_gallery_img`.`move`='0')
            LEFT JOIN `mod_gallery_img_spr` ON (`mod_gallery_img_spr`.`cod`=`mod_gallery_img`.`id_position` AND `mod_gallery_img_spr`.`lang_id`='$this->lang_id')
	    , `".TblModGalleryTxt."`, `".TblModGalleryCat."`
        ...
        ";
        if(!empty($Gallery->category)) $q .= "AND `category`='".$Gallery->category."'";
    ...
    function Link( $cat , $id = NULL)
    {
        ...
        if($cat!=NULL and $id==NULL)
            return _LINK.'proekti/'.$cat.'/';
        elseif($id!=NULL)
            return _LINK.'proekti/'.$cat.'/'.$id.'.html';
        else
            return _LINK.'proekti/';
